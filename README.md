...
カレンダーには選択中の通貨ペアに含まれるSP日数を具体的に表示します。保有数量が設定されている通貨ペアは表示順の先頭へ移動し、カレンダーにはSP日数に代えて、その日の保有分を合計した受払額と月間合計を表示します。月間合計では未発表を含む日を除外し、金額が確定している日だけを合算します。設定画面では通貨ペアの表示順・表示有無・保有通貨数量・買売ポジションを保存できます。掲載値は1万通貨あたりとして、GMO外貨の端数処理に合わせて円単位へ丸めます。

ハンバーガーメニューの「保有数量計算」では、入金額・含み損・目標レバレッジとGMO外貨から取得した為替相場を使い、目標レバレッジに収まる保有通貨数量を計算できます。クロス円以外の通貨ペアも基準通貨の対円レートで円換算します。
