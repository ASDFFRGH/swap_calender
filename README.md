...
ハンバーガーメニューの「保有数量計算」では、複数通貨ペアの保有数量・通貨ペア別の含み損益・入金額とGMO外貨から取得した為替相場を使い、口座全体の現在レバレッジと、目標レバレッジまで追加保有できる数量を計算できます。クロス円以外の通貨ペアも基準通貨・決済通貨の対円レートで円換算します。ロスカット計算は個人口座の維持証拠金率4%と証拠金維持率50%を前提とする参考値です。
入力した通貨ペア別の保有数量・含み損益・売買方向、入金額、目標レバレッジは端末内に保存され、アプリを終了しても次回起動時に復元されます。
