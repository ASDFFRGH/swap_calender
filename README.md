...
カレンダーには選択中の通貨ペアに含まれるSP日数を具体的に表示します。保有数量が設定されている通貨ペアは表示順の先頭へ移動し、カレンダーにはSP日数に代えて、その日の保有分を合計した受払額を表示します。設定画面では通貨ペアの表示順・表示有無・保有通貨数量・買売ポジションを保存できます。掲載値は1万通貨あたりとして、GMO外貨の端数処理に合わせて円単位へ丸めます。
